...
/**
    ...
    public function createTppCredentialRequest($tpp_credentials_params, $x_request_id = null, string $contentType = self::contentTypes['createTppCredential'][0])
    {
        ...
        if (isset($tpp_credentials_params)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                ...
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($tpp_credentials_params));
            } else {
    ...
    public function editTppCredentialRequest($id, $edit_tpp_credential_params, $x_http_method_override = null, $x_request_id = null, string $contentType = self::contentTypes['editTppCredential'][0])
    {
        ...
        if (isset($edit_tpp_credential_params)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($edit_tpp_credential_params));
            } else {
     ...
     *
     * @param  string $search Returns only the TPP credentials belonging to those banks whose &#39;name&#39;, &#39;blz&#39;, or &#39;bic&#39; contains the given search string (the matching works case-insensitive). You may also pass an IBAN in this field, in which case finAPI will try to detect the related bank and regard only this bank for the search (The IBAN may not contain spaces). If the given search string consists of several terms (separated by whitespace), then ALL of these terms must apply to a bank for it to get regarded for the result. (optional)
